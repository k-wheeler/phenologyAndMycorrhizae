...
transitions <- c(15,50,85)
t=1

par(mfrow=c(2,3))
for(drt in c("rising","falling")){
  for(t in seq_along(transitions)){
    allPCdat_cleaned_trans <- allPCdat_cleaned %>% 
      filter(direction==drt,transition==transitions[t],veg_type=="DB")
    allBioDat_plotFiltered_trans <- left_join(allBioDat_plotFiltered,allPCdat_cleaned_trans,by=c('year','siteID'))
    
    mdl <- lm(GDD_5~biodiversity,data = allBioDat_plotFiltered_trans)
    sm <- summary(mdl)
    plot(allBioDat_plotFiltered_trans$biodiversity,allBioDat_plotFiltered_trans$GDD_5,pch=20,
         ylab="Biodiversity",xlab=paste0("GDD_5 at ",transitions[t],"% ",drt),main=paste("p-value on slope:",round(sm$coefficients[2,4],digits=3)))
