...
for(s in seq_along(NEON_siteNames)){
  ...
  dir.create(paste0(dataPath,'NEON_PrecipitationData/',NEON_siteNames[s]))
}
...
for(s in seq_along(NEON_siteNames)[2:47]){
  print(s)
  savePath <- paste0(dataPath,'NEON_PrecipitationData/',NEON_siteNames[s])
  zipsByProduct(dpID='DP1.00006.001',site=NEON_siteNames[s],
                savepath=savePath,check.size = F)
  stackByTable(paste0(savePath,'/filesToStack00006'))
}
...
for(s in seq_along(NEON_siteNames)[1:47]){
  ...
  if(file.exists(paste0(dataPath,"/NEON_PrecipitationData/",NEON_siteNames[s],'/filesToStack00006/stackedFiles/PRIPRE_30min.csv'))){
    subDat <- read.csv(paste0(dataPath,"/NEON_PrecipitationData/",NEON_siteNames[s],'/filesToStack00006/stackedFiles/PRIPRE_30min.csv'))
